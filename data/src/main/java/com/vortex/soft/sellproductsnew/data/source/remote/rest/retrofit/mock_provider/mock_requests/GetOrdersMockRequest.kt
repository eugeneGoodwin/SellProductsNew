...
import com.vortex.soft.sellproductsnew.data.source.remote.rest.retrofit.mock_provider.mock_requests.responses.GetOrdersMockResponse

class GetOrdersMockRequest : MockRequest {
    override fun isMatch(requestType: String, url: String, requestBodyJson: String): Boolean {
        return requestType.toLowerCase() == "get" && url.contains("api/get-orders")
    }

    override fun response(): MockResponse = GetOrdersMockResponse()
}